...
| `executor` | コマンド実行・パイプライン接続・`if`/`elif`/`else`/`fi`・`for`/`while`/`until` ループ。展開パイプライン: コマンド置換 → チルダ → ブレース → glob | ビルトイン in-process、スタック配列 |
...
- **ヒアドキュメント `<<DELIM`**: 複数行テキストを stdin に供給（インタラクティブ・スクリプト両対応）
- **ヒアストリング `<<<word`**: 単一文字列を stdin に供給
- **`pushd` / `popd` / `dirs`**: ディレクトリスタック管理（pushd で積んで移動、popd でポップして移動、dirs で一覧）
